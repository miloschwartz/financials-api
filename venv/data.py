...
def load_income_stmt(ticker):
    ...
    if os.path.exists(path) is False:
        ...
        with open("income_stmts/" + ticker + "_income_stmt.json", "w") as f:
            f.write(json.dumps(income_stmt, indent=4))
    else:
        print(path + " already cached.")
        with open("income_stmts/" + ticker + "_income_stmt.json") as f:
...
def load_cash_flow_stmt(ticker):
    ...
    if os.path.exists(path) is False:
        ...
        with open("cash_flow_stmts/" + ticker + "_cash_flow_stmt.json", "w") as f:
            f.write(json.dumps(cash_flow_stmt, indent=4))
    else:
        print(path + " already cached.")
        with open("cash_flow_stmts/" + ticker + "_cash_flow_stmt.json") as f:
...
def load_balance_sheet(ticker):
    ...
    if os.path.exists(path) is False:
        ...
        with open("balance_sheets/" + ticker + "_balance_sheet.json", "w") as f:
            f.write(json.dumps(balance_sheet, indent=4))
    else:
        print(path + " already cached.")
        with open("balance_sheets/" + ticker + "_balance_sheet.json") as f:
